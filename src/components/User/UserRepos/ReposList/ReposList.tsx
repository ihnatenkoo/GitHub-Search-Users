...
const ReposList: FC<IReposList> = ({ repo }) => {
	...
	const [isFav, setIsFav] = useState<boolean>(
		favRepos.some((favRepo) => favRepo.id === repo.id)
	);
		...
		>
			...
			>
				<div className="flex items-center flex-col sml:mr-5 mr-2 text-center text-sm max-w-[75px]">
					...
					/>
					<span className="min-w-[65px] text-sm">
						{checkTextValue(repo.language)}
					</span>
				</div>

				<article className="w-[425px]">
					<p className="mb-0.5 font-bold text-sm sml:text-base">
						{repo.full_name}
					</p>
			...
			<SwitchTransition mode={'out-in'}>
				<CSSTransition key={isFav ? 'remove' : 'add'} timeout={150}>
					...
					>
						{isFav ? (
							<RemoveFavoriteBtn id={repo.id} />
						) : (
							<AddFavoriteBtn item={repo} />
						)}
					</div>
				</CSSTransition>
			</SwitchTransition>
